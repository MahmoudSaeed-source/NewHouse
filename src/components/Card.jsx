...
const Card = ({ house }) => {
  return (
    <div className="w-full h-full  flex items-center flex-col mb-2  shadow-sm bg-white dark:shadow-white  shadow-black  relative ">
      <div className=" group text-2xl w-full h-auto flex  items-center  pt-1 relative">
        <img className="w-full h-auto " src={house.photo} alt="images" />
      </div>
      ...
      <h2 className="group text-gray-800 w-full h-10  flex justify-start items-center  font-links text-[12px]   px-4">
        <Link
          to={`/HousesForSale/${house.property_id}`}
        >
          Read More
        </Link>
